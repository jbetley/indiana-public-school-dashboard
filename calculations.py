...
def find_nearest(
    ...
    # drop rows missing Lat/Lon data
...
def get_distance(
    ...
    # gets a list of the indexes and distances in the data tree that
    # match the [num_hits] number of "nearest neighbor" schools
    distance, index = tree.query(data.iloc[school_idx][["x", "y", "z"]], k=num)

    index_list = index[0].tolist()
    distance_list = distance[0].tolist()

    # TODO: Only do this when index_list is an actual list? Why is this being called
    # TODO: on a single school?
    # Match School ID with indexes
    closest_schools = pd.DataFrame()
    closest_schools["School ID"] = data[data.index.isin(index_list)]["School ID"]
